...
class AutoUNet_A(pl.LightningModule):
    """
    Goal : Fine-tune the autoencoder with additinoal skip connection
    """
    ...
    def forward(self, x, x_gt, sample_posterior=False):

        # ENCODING
        with torch.no_grad():
            posterior_gt = self.encode_fix(x)
        posterior, intermids = self.encode(x)

        # FEATURE SAMPLE
        if sample_posterior:
            z_gt = posterior_gt.sample()
            z = posterior.sample()
        else:
        ...
        # DECODING
        x_hat = self.decode(z_gt, intermids)
        x_hat_prime = self.decode_fix(z)

        return x_hat, x_hat_prime, posterior, posterior_gt
    ...
    def training_step(self, batch, batch_idx):
        ...
        x_hat, x_hat_prime, *_ = self(img_lf, img_gt)
